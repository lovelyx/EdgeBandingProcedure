...
from afterEnd.page import style

class My_sheet(object):
    def __init__(self, sheet_name='优化数据', re_write=True):
        '''
        自定义类说明：
    ...
    def Day(self,numList,numListDay,filname):
        ...
        # for i in range(0,len(numListDay)):
            ...
            if j == len(numListDay) - 1:
                ...
                self.write_merge(numDay2, numDay, 9, 9, self.MaterialresultSum, style.style.style2(self=""))
                self.write_merge(numDay2, numDay, 0, 0, numListDay[j], style.style.style0(self=""))
                break
    ...
    def totalFile(self,num,start_row,style=style.style.style3(self='')):
        '''
        向表中添加合计数据
        :param num: 表中一共又多少行
